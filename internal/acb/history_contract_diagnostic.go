...
func (c *Client) logHistoryContract(phase string, fields map[string]string, response Response) {
	if response.Kind != HistoryPage && response.Kind != AccountDetailPage {
		return
	}
	key := phase + ":" + safeHistoryControl("FromDate", fields["FromDate"]) + ":" + safeHistoryControl("ToDate", fields["ToDate"])
	...
	if doc, err := html.Parse(strings.NewReader(response.Body)); err == nil {
		walk(doc, func(n *html.Node) {
			...
			name := attrVal(n, "name")
			switch name {
			case "FromDate", "ToDate", "activeDatetimeYN", "activeDatetimeByMonth", "CheckRef", "CheckDoiUng", "dse_nextEventName", "virtualAccount":
				var labels []string
				if n.Parent != nil {
					text := strings.ToLower(domVisibleText(n.Parent))
					for _, label := range []string{"hôm nay", "hom nay", "từ ngày", "tu ngay", "đến ngày", "den ngay", "theo ngày", "theo ngay", "theo tháng", "theo thang", "gần nhất", "gan nhat"} {
				...
				controls[name] = append(controls[name], map[string]any{
					...
					"radio":    strings.EqualFold(attrVal(n, "type"), "radio"),
					"checkbox": strings.EqualFold(attrVal(n, "type"), "checkbox"),
					"hidden":   strings.EqualFold(attrVal(n, "type"), "hidden"),
					"submit":   strings.EqualFold(attrVal(n, "type"), "submit"),
					"labels":   labels,
				})
			}
		})
	...
	for _, name := range []string{"FromDate", "ToDate", "activeDatetimeYN", "activeDatetimeByMonth", "CheckRef", "CheckDoiUng", "dse_nextEventName", "virtualAccount"} {
