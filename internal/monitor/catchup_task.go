...
func (t *CatchUpTask) ensurePoll(ctx context.Context) error {
	...
	if t.explicitRecovery {
		poll.Classifier = "RECOVERY"
	} else {
		poll.Classifier = "CATCH_UP"
	}
	t.poll = poll
	return nil
}

func (t *CatchUpTask) finishPoll(ctx context.Context, status, pollErr string) error {
	...
	if t.poll.Classifier == "" {
		if t.explicitRecovery {
			t.poll.Classifier = "RECOVERY"
		} else {
			t.poll.Classifier = "CATCH_UP"
		}
	}
